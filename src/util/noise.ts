...
const Permute = [151, 160, 137, 91, 90, 15,
...
// 2D Perlin Noise
export function perlin2(seed: number = 0) {

  const { perm, gradP } = rands(seed);
...
// 3D Perlin Noise
export function perlin3(seed: number = 0) {
...
export function simplex2(seed: number) {
  ...
  return function (xin: number, yin: number) {
    ...
    const x0 = xin - i + (i + j) * G2; // The x,y distances from the cell origin, unskewed.
    const y0 = yin - j + (i + j) * G2;
    ...
    let tmp = 0.5 - x0 * x0 - y0 * y0;    // t0
    if (tmp >= 0) {
      tmp *= tmp;
      const gi0 = gradP[i + perm[j]];
      out = tmp * tmp * gi0.dot2(x0, y0);  // (x,y) of grad3 used for 2D gradient
    }
    tmp = 0.5 - x1 * x1 - y1 * y1;    // t1
    if (tmp >= 0) {
      tmp *= tmp;
      const gi1 = gradP[i + i1 + perm[j + j1]];
      out += tmp * tmp * gi1.dot2(x1, y1);
    }
    tmp = 0.5 - x2 * x2 - y2 * y2;      // t2
    if (tmp >= 0) {
      tmp *= tmp;
      const gi2 = gradP[i + 1 + perm[j + 1]];
      out = tmp * tmp * gi2.dot2(x2, y2);
    }
...
// 3D simplex noise
export function simplex3(seed: number = 0) {
  const { perm, gradP } = rands(seed);
  return function (xin: number, yin: number, zin: number) {
    ...
    let tmp = 0.5 - x0 * x0 - y0 * y0 - z0 * z0;  //t0
    if (tmp >= 0) {
      tmp *= tmp;
      const gi0 = gradP[i + perm[j + perm[k]]];
      out = tmp * tmp * gi0.dot3(x0, y0, z0);  // (x,y) of grad3 used for 2D gradient
    }
    tmp = 0.5 - x1 * x1 - y1 * y1 - z1 * z1;  // t1
    if (tmp >= 0) {
      tmp *= tmp;
      const gi1 = gradP[i + i1 + perm[j + j1 + perm[k + k1]]];
      out += tmp * tmp * gi1.dot3(x1, y1, z1);
    }
    tmp = 0.5 - x2 * x2 - y2 * y2 - z2 * z2; // t2
    if (tmp >= 0) {
      tmp *= tmp;
      const gi2 = gradP[i + i2 + perm[j + j2 + perm[k + k2]]];
      out += tmp * tmp * gi2.dot3(x2, y2, z2);
    }
    tmp = 0.5 - x3 * x3 - y3 * y3 - z3 * z3; // t3
    if (tmp >= 0) {
      tmp *= tmp;
      const gi3 = gradP[i + 1 + perm[j + 1 + perm[k + 1]]];
      out += tmp * tmp * gi3.dot3(x3, y3, z3);
    }
    // Add contributions from each corner to get the final noise value.
    // The result is scaled to return values in the interval [-1,1].
    return 32 * out;
